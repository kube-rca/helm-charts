...
| chunksCache.addresses | string | `"dnssrvnoa+_memcached-client._tcp.{{ include \"loki.resourceName\" (dict \"ctx\" $ \"component\" \"chunks-cache\" \"suffix\" $.Values.chunksCache.suffix ) }}.{{ include \"loki.namespace\" $ }}.svc.{{ .Values.global.clusterDomain }}"` | Comma separated addresses list in DNS Service Discovery format |
...
| chunksCache.l2.persistence | object | `{"enabled":false,"labels":{},"mountPath":"/data","storageClass":null,"storageSize":"10G"}` | Persistence settings for the chunks-cache-l2 |
...
| chunksCache.persistence | object | `{"enabled":false,"labels":{},"mountPath":"/data","storageClass":null,"storageSize":"10G"}` | Persistence settings for the chunks-cache |
...
| enterprise.image.tag | string | `"3.5.4"` | Docker image tag |
...
| enterprise.provisioner.image | object | `{"digest":null,"pullPolicy":"IfNotPresent","registry":"us-docker.pkg.dev","repository":"grafanalabs-global/docker-enterprise-provisioner-prod/enterprise-provisioner","tag":"latest"}` | Provisioner image to Utilize |
...
| enterprise.provisioner.image.registry | string | `"us-docker.pkg.dev"` | The Docker registry |
| enterprise.provisioner.image.repository | string | `"grafanalabs-global/docker-enterprise-provisioner-prod/enterprise-provisioner"` | Docker image repository |
| enterprise.provisioner.image.tag | string | `"latest"` | Overrides the image tag whose default is the chart's appVersion |
...
| ingester.serviceType | string | `"ClusterIP"` | Service type for ingestor service |
...
| loki.commonConfig | object | `{"compactor_grpc_address":"{{ include \"loki.compactorAddress\" . }}","path_prefix":"/var/loki","replication_factor":3}` | Check https://grafana.com/docs/loki/latest/configuration/#common_config for more info on how to provide a common configuration |
...
| loki.image.tag | string | `"3.5.7"` | Overrides the image tag whose default is the chart's appVersion |
...
| lokiCanary.kind | string | `"DaemonSet"` | The type of the loki canary k8s rollout. This can be a DaemonSet or Deployment. |
...
| monitoring.serviceMonitor | object | `{"annotations":{},"enabled":false,"interval":"15s","labels":{},"metricRelabelings":[],"metricsInstance":{"annotations":{},"enabled":true,"labels":{},"remoteWrite":null},"namespaceSelector":{},"relabelings":[],"scheme":"http","scrapeTimeout":null,"tlsConfig":null}` | ServiceMonitor configuration |
...
| monitoring.serviceMonitor.metricsInstance | object | `{"annotations":{},"enabled":true,"labels":{},"remoteWrite":null}` | DEPRECATED If defined, will create a MetricsInstance for the Grafana Agent Operator. |
...
| read.persistence | object | `{"accessModes":["ReadWriteOnce"],"annotations":{},"enableStatefulSetAutoDeletePVC":true,"labels":{},"selector":null,"size":"10Gi","storageClass":null}` | read.persistence is used only if legacyReadTarget is set to true |
...
| resultsCache.addresses | string | `"dnssrvnoa+_memcached-client._tcp.{{ include \"loki.resourceName\" (dict \"ctx\" $ \"component\" \"results-cache\") }}.{{ include \"loki.namespace\" $ }}.svc.{{ .Values.global.clusterDomain }}"` | Comma separated addresses list in DNS Service Discovery format |
...
| resultsCache.persistence | object | `{"enabled":false,"labels":{},"mountPath":"/data","storageClass":null,"storageSize":"10G"}` | Persistence settings for the results-cache |
...
| sidecar.image.repository | string | `"docker.io/kiwigrid/k8s-sidecar"` | The Docker registry and image for the k8s sidecar |
| sidecar.image.sha | string | `""` | Docker image sha. If empty, no sha will be used |
| sidecar.image.tag | string | `"1.30.10"` | Docker image tag |
...
| test | object | `{"annotations":{},"canaryServiceAddress":"http://{{ include \"loki-canary.fullname\" $ }}.{{ include \"loki.namespace\" $ }}.svc.{{ .Values.global.clusterDomain }}:3500/metrics","enabled":true,"hostUsers":"nil","image":{"digest":null,"pullPolicy":"IfNotPresent","registry":"docker.io","repository":"grafana/loki-helm-test","tag":"latest"},"labels":{},"prometheusAddress":"","timeout":"1m"}` | Section for configuring optional Helm test |
...
| test.image | object | `{"digest":null,"pullPolicy":"IfNotPresent","registry":"docker.io","repository":"grafana/loki-helm-test","tag":"latest"}` | Image to use for loki canary |
...
| test.image.tag | string | `"latest"` | Overrides the image tag whose default is the chart's appVersion |
