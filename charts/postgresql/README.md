...
![Version: 18.1.13](https://img.shields.io/badge/Version-18.1.13-informational?style=flat-square) ![AppVersion: 18.1.0](https://img.shields.io/badge/AppVersion-18.1.0-informational?style=flat-square)
...
|------------|------|---------|
| oci://registry-1.docker.io/bitnamicharts | common | 2.33.2 |
...
| auth.username | string | `"kube-rca"` |  |
| backup.cronjob.annotations | object | `{}` |  |
| backup.cronjob.command[0] | string | `"/bin/bash"` |  |
| backup.cronjob.command[1] | string | `"-c"` |  |
| backup.cronjob.command[2] | string | `"PGPASSWORD=\"${PGPASSWORD:-$(< \"$PGPASSWORD_FILE\")}\" pg_dumpall --clean --if-exists --load-via-partition-root --quote-all-identifiers --no-password --file=\"${PGDUMP_DIR}/pg_dumpall-$(date '+%Y-%m-%d-%H-%M').pgdump\""` |  |
...
| backup.cronjob.fips.openssl | string | `""` |  |
...
| global.postgresql.auth.database | string | `"kube-rca"` |  |
...
| global.postgresql.auth.username | string | `"kube-rca"` |  |
...
| image.registry | string | `"registry-1.docker.io"` |  |
...
| metrics.fips.golang | string | `"relaxed"` |  |
...
| readReplicas.fips.openssl | string | `""` |  |
